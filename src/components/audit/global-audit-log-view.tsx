...
export function GlobalAuditLogView({ initialLogs }: GlobalAuditLogViewProps) {
  ...
  const filteredLogs = useMemo(() => {
    return initialLogs.filter((log) => {
      ...
      // 3. Date range filter
      if (startDate) {
        const logDate = log.changed_at.split('T')[0]
  ...
  const hasActiveFilters = moduleFilter !== 'all' || actionFilter !== 'all' || startDate !== '' || endDate !== '' || search !== ''
    ...
    <div className="flex flex-col gap-5 max-w-4xl mx-auto">
      ...
      <div className="p-4 rounded-xl bg-white dark:bg-[#121215] border border-[#E5E7EB] dark:border-[#27272A] flex flex-col gap-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
          <div className="relative sm:col-span-2">
            ...
            <input
              type="text"
              placeholder={t.auditLog.searchPlaceholder || 'Cari riwayat, akun, kategori, nominal...'}
              value={search}
      ...
      {filteredLogs.length === 0 ? (
        <div className="p-8 text-center bg-white dark:bg-[#121215] rounded-xl border border-[#E5E7EB] dark:border-[#27272A] text-xs text-[#94A3B8]">
          {t.auditLog.emptySearch || 'Tidak ada catatan audit yang cocok dengan kriteria filter.'}
        </div>
      ) : (
