...
export function DebtPaymentModal({
  ...
  useEffect(() => {
    if (isOpen && debt) {
      ...
      const defaultMatch = matchingAccounts.find((a) => a.id === savedDefaultId)
      if (defaultMatch) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        setError(res.error)
        toast.error(t.debts.createPaymentFailed || (language === 'en' ? 'Failed to Record Payment' : 'Gagal Mencatat Cicilan'), { description: res.error })
      } else {
        toast.success(language === 'en' ? 'Installment recorded successfully' : 'Cicilan berhasil dicatat & saldo akun terpotong')
        setAmount('')
        onSuccess?.()
        onClose()
      }
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
        ...
        <div className="flex flex-col gap-1.5">
            ...
            <div className="p-3 rounded-xl bg-[#FFFBEB] dark:bg-[#78350F]/20 border border-[#FDE68A] dark:border-[#92400E]/40 text-xs text-[#B45309] dark:text-[#FDE68A] flex items-start gap-2">
              ...
              <span>
                {t.debts.noMatchingAccount
                  ? t.debts.noMatchingAccount.replace('{currency}', debt?.currency || '')
          ...
          <p className="text-[10px] text-[#64748B] dark:text-[#94A3B8]">
            {debt?.type === 'debt'
              ? (t.debts.accountDeductNote || 'Saldo rekening terpilih akan otomatis terpotong (Pengeluaran).')
              : (t.debts.accountCreditNote || 'Saldo rekening terpilih akan otomatis bertambah (Pemasukan).')}
          </p>
        </div>
