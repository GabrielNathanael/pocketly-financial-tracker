...
import { Account, EnrichedSavingsGoal, GoalDepositType } from '@/types/database'
...
export function GoalDepositModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(numAmount) || numAmount <= 0) {
      setErrorMsg(language === 'en' ? 'Please enter a valid amount' : 'Nominal harus lebih dari 0')
      return
    }

    if (!accountId) {
      setErrorMsg(language === 'en' ? 'Please select an account (Cash or Bank)' : 'Silakan pilih akun sumber/tujuan (Kas Tunai atau Bank)')
      return
    }
    ...
    >
      <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
        ...
        <Input
          ...
          rightIcon={<span className="text-xs font-mono font-bold text-[#94A3B8]">{goal.currency}</span>}
        />
        ...
        {/* Mandatory Account Selector (Bank, E-Wallet, or Cash) */}
        <div className="flex flex-col gap-1.5">
          <div className="flex items-center justify-between">
            <label className="text-[10px] font-bold uppercase tracking-wider text-[#64748B] dark:text-[#94A3B8]">
              {type === 'deposit'
                ? language === 'en' ? 'Source Account (Bank / Cash)' : 'Rekening Sumber (Bank / Kas Tunai)'
                : language === 'en' ? 'Destination Account (Bank / Cash)' : 'Rekening Tujuan (Bank / Kas Tunai)'}
            </label>
        ...
        <div className="pt-3 border-t border-[#E5E7EB] dark:border-[#27272A] flex items-center justify-end gap-2">
          ...
          >
            {loading ? t.common.loading : type === 'deposit' ? t.goals.depositBtn : t.goals.withdrawBtn}
          </Button>
        </div>
      </form>
    </Modal>
