...
async function syncStockHolding(
  ...
  for (const tr of trades) {
    ...
    if (tr.type === "buy") {
      ...
      const costDeducted =
        Number(tr.buy_cost) ||
        (runningLots > 0 ? (trLots / runningLots) * runningCost : 0);
 ...
 */
export async function getInvestmentTradesHistory(): Promise<
  EnrichedStockTrade[]
> {
  const supabase = await createServerSupabaseClient();
  const {
 ...
 */
export async function getDailyTradingVolume(
  accountId: string,
  dateStr: string,
) {
  const supabase = await createServerSupabaseClient();
...
export async function recordStockBuy(input: {
  ...
  const { dailyVolume, stampDutyCharged } = await getDailyTradingVolume(
    acc.id,
    dateOnly,
  );
  ...
  const txDate = input.tradeDate
    ? `${input.tradeDate}T12:00:00.000Z`
    : new Date().toISOString();

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { error: tradeErr } = await (
    supabase.from("stock_trades") as any
  ).insert({
    user_id: user.id,
    account_id: acc.id,
    holding_id: null,
  ...
  // 4. Record Stamp Duty as Expense Transaction if applied
  if (stampDutyToApply > 0) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: stampCat } = await (supabase.from("categories") as any)
      .select("id")
      .eq("user_id", user.id)
      .eq("name", "Stamp Duty")
      .eq("type", "expense")
    ...
    if (stampCat) {
      ...
      const stampDesc = isEn
        ? `Stock Trading Stamp Duty (${acc.name})`
        : `Bea Materai Transaksi Saham (${acc.name})`;

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (supabase.from("transactions") as any).insert({
        user_id: user.id,
        account_id: acc.id,
        category_id: stampCat.id,
        type: "expense",
        amount: stampDutyToApply,
...
export async function recordStockSell(input: {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: holding, error: holdErr } = await (
    supabase.from("stock_holdings") as any
  )
    .select("*, account:accounts(*)")
  ...
  // 5. Record Stamp Duty as Expense Transaction if applied
  if (stampDutyToApply > 0) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: stampCat } = await (supabase.from("categories") as any)
      .select("id")
...
export async function deleteStockTrade(id: string) {
  ...
  // 1. Fetch trade first
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: trade, error: fetchErr } = await (
    supabase.from("stock_trades") as any
  )
    .select("*, account:accounts(*)")
    .eq("id", id)
...
export async function updateStockTrade(input: UpdateStockTradeInput) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: trade, error: tradeErr } = await (
    supabase.from("stock_trades") as any
  )
    .select("*, account:accounts(*)")
    .eq("id", input.id)
  ...
  const acc = trade.account;
  const newLots =
    input.lots !== undefined ? Number(input.lots) : Number(trade.lots) || 1;
  ...
  if (delta !== 0 && acc) {
    ...
    if (trade.type === "buy") {
      if (currentBal - delta < 0) {
        return {
          error: `Saldo RDN tidak mencukupi untuk penyesuaian nominal (${acc.name})`,
        };
      }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (supabase.from("accounts") as any)
