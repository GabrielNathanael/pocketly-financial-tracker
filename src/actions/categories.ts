...
export async function deduplicateUserCategories(userId?: string) {
  ...
  const { data: allCategories, error } = await (
  ...
  for (const [, catList] of canonicalMap.entries()) {
    const canonicalEnglishName =
      getCanonicalCategoryName(catList[0].name) || catList[0].name;
    ...
    if (catList.length > 1) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (supabase.from("categories") as any).delete().in("id", deleteIds);
...
export async function seedUserDefaultCategories(userId?: string) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { data: existingCategories } = await (
    supabase.from("categories") as any
  )
    .select("id, name, type")
    .eq("user_id", targetUserId);
    ...
    existingCategories.some(
      ...
      (c: any) =>
        ...
        (c.name?.toLowerCase() === name.toLowerCase() ||
          getCanonicalCategoryName(c.name).toLowerCase() ===
            name.toLowerCase()),
    );
  ...
  if (!checkHasCategory("Stamp Duty", "expense")) {
    missingCategories.push({
      user_id: targetUserId,
      name: "Stamp Duty",
      type: "expense",
...
export async function deleteCategory(id: string) {
  ...
  if (count && count > 0) {
    return {
      error: `Kategori ini tidak dapat dihapus karena masih digunakan oleh ${count} catatan transaksi. Ubah atau hapus transaksi terkait terlebih dahulu.`,
    };
  }
