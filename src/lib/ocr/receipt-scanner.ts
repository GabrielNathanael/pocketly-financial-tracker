...
export function parseReceiptText(text: string): ParsedReceipt {
  ...
  for (let i = 0; i <= Math.min(headerEndIdx, lines.length - 1); i++) {
    ...
    const isMeta =
      ...
      /^[0-9\s.,\-_/:*#]+$/.test(lines[i]);

    if (!isAddr && !isMeta && lines[i].length >= 3) {
      let cleanName = lines[i]
        .replace(/[^\w\s&.'-]/gi, "")
        .replace(/\s+/g, " ")
        .trim();
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const kw of totalKeywords) {
      if (lineLower.includes(kw)) {
        const priceMatches = lineLower.match(
          /(?:rp\d.?|idr|\$)?\s*([0-9]{1,3}(?:[.,][0-9]{3})+|[0-9]+(?:[.,][0-9]{2})?|[0-9]{4,})/gi,
        );
        if (priceMatches) {
          const amt = extractPrice(priceMatches[priceMatches.length - 1]);
          if (amt > 0) {
            const isMainTotal =
              kw.includes("bayar") || kw.includes("grand") || kw === "total";
    ...
    if (i >= summaryStartIdx) {
      if (
        ...
        lineLower.includes("bayar")
      ) {
        if (!lineLower.includes("kembali") && !lineLower.includes("change")) {
          const priceMatches = lineLower.match(
            /(?:rp\.?|idr|\$)?\s*([0-9]{1,3}(?:[.,][0-9]{3})+|[0-9]+(?:[.,][0-9]{2})?|[0-9]{4,})/gi,
          );
          if (priceMatches) {
            const amt = extractPrice(priceMatches[priceMatches.length - 1]);
  ...
  for (const line of itemCandidateLines) {
    ...
    const itemMatch = line.match(
      /^(.+?)\s+((?:rp\.?|idr|\$)?\s*[0-9]{1,3}(?:[.,][0-9]{3})+|[0-9]+(?:[.,][0-9]{2})?|[0-9]{3,})$/i,
    );
    if (itemMatch) {
      ...
      if (
        itemName.length >= 2 &&
        itemPriceNum >= 100 &&
        itemPriceNum <= (totalAmount || 100000000)
      ) {
        items.push({
          name: itemName,
  ...
  return {
    ...
    date: detectedDate || getLocalDateString(),
    items,
    rawText: text,
  };
}
  ...
  const worker = await createWorker("ind+eng", 1, {
    logger: (m) => {
      if (m.status === "recognizing text") {
        ...
        onProgress?.(
          pct,
          `Membaca karakter struk... ${Math.round(m.progress * 100)}%`,
        );
      }
    },
  });
