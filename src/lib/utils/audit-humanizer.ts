...
export function humanizeAuditLog(
  ...
  if (table === "stock_trades") {
    ...
    const lots =
      newVal.lots ||
      oldVal.lots ||
      Number(newVal.shares || oldVal.shares) / 100 ||
      0;
    ...
    const pricePerShare = Number(
      newVal.price_per_share ||
        oldVal.price_per_share ||
        (shares > 0 ? netAmount / shares : 0),
    );
      ...
      return {
        ...
        changes: [
            ...
            ? [
                {
                  ...
                  to: `${lots} Lot (${shares.toLocaleString()} lbr)`,
                },
              ]
            : []),
          ...(pricePerShare > 0
            ? [
                {
                  field: isId ? "Harga per Lembar" : "Price / Share",
                  to: formatCurrency(Math.round(pricePerShare), "IDR"),
                },
              ]
            : []),
          ...
          ...(!isBuy && realizedPnl !== 0
            ? [
                {
                  field: isId ? "Realized PnL" : "Realized P&L",
                  to: `${realizedPnl >= 0 ? "+" : ""}${formatCurrency(realizedPnl, "IDR")}`,
                },
              ]
            : []),
          ...(stampDuty > 0
            ? [
                {
                  field: isId ? "Bea Materai" : "Stamp Duty",
                  to: formatCurrency(stampDuty, "IDR"),
                },
              ]
            : []),
          ...(newVal.trade_date
    ...
    if (log.action === "DELETE") {
      return {
        ...
        changes: [
          ...
          {
            field: isId ? "Nominal Dihapus" : "Deleted Amount",
            from: formattedNet,
          },
        ],
        badgeType: "delete",
  ...
  if (table === "stock_holdings") {
    ...
    const avgPrice = formatCurrency(
      Number(newVal.avg_buy_price || oldVal.avg_buy_price || 0),
      "IDR",
    );
    ...
    if (log.action === "INSERT") {
      return {
        ...
        changes: [
          ...
          {
            field: isId ? "Harga Rata-Rata" : "Avg Buy Price",
            to: `${avgPrice} / lbr`,
          },
        ],
        badgeType: "create",
    ...
    if (log.action === "DELETE") {
      return {
        ...
        changes: [
          { field: isId ? "Kode Saham" : "Ticker", from: ticker },
          {
            field: isId ? "Modal Sebelumnya" : "Previous Cost",
            from: totalCost,
          },
        ],
        badgeType: "delete",
    ...
    if (log.action === "UPDATE") {
      return {
        ...
        changes: [
          ...
          {
            field: isId ? "Total Modal" : "Total Capital",
            from: formatCurrency(Number(oldVal.total_cost || 0), "IDR"),
            to: totalCost,
          },
          {
            field: isId ? "Harga Rata-Rata" : "Avg Buy Price",
            from: `${formatCurrency(Number(oldVal.avg_buy_price || 0), "IDR")} / lbr`,
            to: `${avgPrice} / lbr`,
          },
        ],
        badgeType: "update",
  ...
  if (table === "transfers") {
    ...
    if (log.action === "INSERT") {
      const fromName =
        newVal.from_account_name || (isId ? "Akun Asal" : "Source Account");
      const toName =
        newVal.to_account_name ||
        (isId ? "Akun Tujuan" : "Destination Account");
      const isCross = fromCur !== toCur;
      const routeLabel = isCross
        ? `${fromName} → ${toName} (${fromCur} → ${toCur})`
        : `${fromName} → ${toName}`;
      ...
      return {
        ...
        changes: [
          { field: isId ? "Rute Transfer" : "Transfer Route", to: routeLabel },
          {
            field: isId ? "Nominal Transfer" : "Transfer Amount",
            to: formattedAmt,
          },
          ...(rate && rate !== 1
            ? [
                {
                  field: isId ? "Kurs Digunakan" : "Exchange Rate Used",
                  to: `1 ${fromCur} = ${rate} ${toCur}`,
                },
              ]
            : []),
          ...(newVal.transfer_date
            ? [
                {
                  field: isId ? "Tanggal Transfer" : "Date",
                  to: formatDate(newVal.transfer_date, "d MMM yyyy", lang),
                },
              ]
            : []),
        ],
    ...
    if (log.action === "DELETE") {
      return {
        ...
        changes: [
          {
            field: isId ? "Nominal Dibatalkan" : "Cancelled Amount",
            from: formattedAmt,
          },
        ],
        badgeType: "delete",
  ...
  if (table === "debts") {
    ...
    const person =
      newVal.counterparty_name ||
      oldVal.counterparty_name ||
      (isId ? "Pihak Lain" : "Counterparty");
    ...
    const typeText = isDebt
      ...
      : isId
        ? "Piutang"
        : "Receivable";
    const initAmt = formatCurrency(
      Number(newVal.initial_amount || oldVal.initial_amount || 0),
      cur,
    );
            ...
            ? [
                {
                  ...
                  to: formatDate(newVal.due_date, "d MMM yyyy", lang),
                },
              ]
            : []),
        ],
    ...
    const debtChanges: Array<{ field: string; from?: string; to?: string }> =
      [];
    if (
      oldVal.counterparty_name !== newVal.counterparty_name &&
      (oldVal.counterparty_name || newVal.counterparty_name)
    ) {
      debtChanges.push({
    ...
    if (
      oldVal.due_date !== newVal.due_date &&
      (oldVal.due_date || newVal.due_date)
    ) {
      debtChanges.push({
        field: isId ? "Tenggat Waktu" : "Due Date",
        from: oldVal.due_date
          ? formatDate(oldVal.due_date, "d MMM yyyy", lang)
          : isId
            ? "(Tanpa Tenggat)"
            : "(No Due Date)",
        to: newVal.due_date
          ? formatDate(newVal.due_date, "d MMM yyyy", lang)
          : isId
            ? "(Tanpa Tenggat)"
            : "(No Due Date)",
      });
  ...
  if (table === "debt_payments") {
    ...
    const amt = formatCurrency(
      Number(newVal.amount || oldVal.amount || 0),
      cur,
    );
      ...
      return {
        ...
        changes: [
          ...
          ...(newVal.payment_date
            ? [
                {
                  field: isId ? "Tanggal Bayar" : "Payment Date",
                  to: formatDate(newVal.payment_date, "d MMM yyyy", lang),
                },
              ]
            : []),
          ...(newVal.notes
  ...
  if (table === "accounts") {
    ...
    if (log.action === "DELETE") {
      return {
        ...
        changes: [
          {
            field: isId ? "Nama Akun Dihapus" : "Deleted Account",
            from: accName,
          },
        ],
        badgeType: "delete",
    ...
    if (oldVal.name !== newVal.name && (oldVal.name || newVal.name)) {
      accChanges.push({
        field: isId ? "Nama Akun" : "Account Name",
        from: oldVal.name,
        to: newVal.name,
      });
    }
    if (oldVal.type !== newVal.type && (oldVal.type || newVal.type)) {
      accChanges.push({
        field: isId ? "Tipe Akun" : "Account Type",
        from: oldVal.type,
        to: newVal.type,
      });
    }
  ...
  if (table === "budgets") {
      ...
      return {
        ...
        changes: [
          ...
          ...(period
            ? [
                {
                  field: isId ? "Periode" : "Period",
                  to: formatDate(period, "MMMM yyyy", lang),
                },
              ]
            : []),
        ],
    ...
    // UPDATE budget
    const oldAmt = formatCurrency(
      Number(oldVal.amount || 0),
      oldVal.currency || "IDR",
    );
    const newAmt = formatCurrency(
      Number(newVal.amount || 0),
      newVal.currency || "IDR",
    );
    return {
      moduleName: isId ? "Anggaran" : "Budgets",
      ...
      changes: [
        {
          field: isId ? "Batas Anggaran" : "Budget Limit",
          from: oldAmt,
          to: newAmt,
        },
      ],
  ...
  if (table === "categories") {
    const catName =
      newVal.name || oldVal.name || (isId ? "Kategori" : "Category");
    ...
    if (log.action === "DELETE") {
      return {
        ...
        changes: [
          {
            field: isId ? "Kategori Dihapus" : "Deleted Category",
            from: catName,
          },
        ],
        badgeType: "delete",
    ...
    return {
      ...
      changes: [
        {
          field: isId ? "Nama Kategori" : "Category Name",
          from: oldVal.name,
          to: newVal.name,
        },
      ],
  ...
  if (table === "recurring_transactions") {
    const name =
      newVal.name ||
      oldVal.name ||
      (isId ? "Transaksi Rutin" : "Recurring Transaction");
    const cur = newVal.currency || oldVal.currency || "IDR";
    const amt = formatCurrency(
      Number(newVal.amount || oldVal.amount || 0),
      cur,
    ...
    if (log.action === "INSERT") {
      return {
        ...
        changes: [
          ...
          ...(newVal.next_due_date
            ? [
                {
                  field: isId ? "Jatuh Tempo Pertama" : "First Due Date",
                  to: formatDate(newVal.next_due_date, "d MMM yyyy", lang),
                },
              ]
            : []),
        ],
    ...
    if (
      oldVal.is_active !== newVal.is_active &&
      (oldVal.is_active !== undefined || newVal.is_active !== undefined)
    ) {
      recChanges.push({
        field: isId ? "Status" : "Status",
        from: oldVal.is_active
          ? isId
            ? "Aktif"
            : "Active"
          : isId
    ...
    if (
      oldVal.next_due_date !== newVal.next_due_date &&
      (oldVal.next_due_date || newVal.next_due_date)
    ) {
      recChanges.push({
        field: isId ? "Tanggal Jatuh Tempo" : "Next Due Date",
        from: oldVal.next_due_date
          ? formatDate(oldVal.next_due_date, "d MMM yyyy", lang)
          : "-",
        to: newVal.next_due_date
          ? formatDate(newVal.next_due_date, "d MMM yyyy", lang)
          : "-",
      });
    }
  ...
  if (table === "savings_goals") {
    const name =
      newVal.name || oldVal.name || (isId ? "Target Tabungan" : "Savings Goal");
    const cur = newVal.currency || oldVal.currency || "IDR";
    const targetAmt = formatCurrency(
      Number(newVal.target_amount || oldVal.target_amount || 0),
      cur,
    );
    const currentAmt = formatCurrency(
      Number(newVal.current_amount || oldVal.current_amount || 0),
      cur,
    );
      ...
      return {
        ...
        changes: [
          ...
          ...(newVal.target_date
            ? [
                {
                  field: isId ? "Tenggat Waktu" : "Target Date",
                  to: formatDate(newVal.target_date, "d MMM yyyy", lang),
                },
              ]
            : []),
        ],
    ...
    if (
      oldVal.target_amount !== newVal.target_amount &&
      (oldVal.target_amount || newVal.target_amount)
    ) {
      goalChanges.push({
        field: isId ? "Target Sasaran" : "Target Amount",
        from: oldVal.target_amount
          ? formatCurrency(Number(oldVal.target_amount), cur)
          : "-",
        to: newVal.target_amount
          ? formatCurrency(Number(newVal.target_amount), cur)
          : "-",
      });
    }
    if (
      oldVal.current_amount !== newVal.current_amount &&
      (oldVal.current_amount !== undefined ||
        newVal.current_amount !== undefined)
    ) {
      goalChanges.push({
        field: isId ? "Terkumpul Saat Ini" : "Current Progress",
        from: oldVal.current_amount
          ? formatCurrency(Number(oldVal.current_amount), cur)
          : "-",
        to: newVal.current_amount
          ? formatCurrency(Number(newVal.current_amount), cur)
          : "-",
      });
    }
    if (
      oldVal.target_date !== newVal.target_date &&
      (oldVal.target_date || newVal.target_date)
    ) {
      goalChanges.push({
        field: isId ? "Tenggat Target" : "Target Date",
        from: oldVal.target_date
          ? formatDate(oldVal.target_date, "d MMM yyyy", lang)
          : isId
            ? "(Tanpa Tenggat)"
            : "(No Deadline)",
        to: newVal.target_date
          ? formatDate(newVal.target_date, "d MMM yyyy", lang)
          : isId
            ? "(Tanpa Tenggat)"
            : "(No Deadline)",
      });
  ...
  if (table === "savings_goal_deposits") {
            ...
            ? [
                {
                  ...
                  to: formatDate(newVal.deposit_date, "d MMM yyyy", lang),
                },
              ]
            : []),
        ],
    ...
    if (log.action === "DELETE") {
      return {
        ...
        changes: [{ field: isId ? "Nominal" : "Amount", from: amt }],
        badgeType: "delete",
        badgeLabel: isId ? "Dihapus" : "Deleted",
      };
    }
  ...
  if (log.action === "INSERT") {
    const amount = newVal.amount
      ? formatCurrency(Number(newVal.amount), newVal.currency || "IDR")
      : "";
    const typeLabel =
      newVal.type === "income"
        ? isId
  ...
  if (log.action === "DELETE") {
    const amount = oldVal.amount
      ? formatCurrency(Number(oldVal.amount), oldVal.currency || "IDR")
      : "";
    ...
    return {
      ...
      changes: [
        ...(oldVal.amount
          ...
          : []),
        ...(cleanDesc
          ? [{ field: isId ? "Keterangan" : "Note", from: cleanDesc }]
          : []),
      ],
      badgeType: "delete",
      badgeLabel: isId ? "Dihapus" : "Deleted",
  ...
  if (
    ...
    (oldVal.amount !== undefined || newVal.amount !== undefined)
  ) {
    changes.push({
  ...
  if (oldTags !== newTags) {
    changes.push({
      field: isId ? "Tagar (#tags)" : "Tags",
      from:
        oldVal.tags && oldVal.tags.length > 0
          ? oldVal.tags.map((t: string) => `#${t}`).join(", ")
          : isId
            ? "(Tanpa Tagar)"
            : "(No Tags)",
      to:
        newVal.tags && newVal.tags.length > 0
          ? newVal.tags.map((t: string) => `#${t}`).join(", ")
          : isId
            ? "(Tanpa Tagar)"
            : "(No Tags)",
    });
